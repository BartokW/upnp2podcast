...
sub getcontentlist {
  ...
  #print "*****\n$result\n******\n";
	...
	while ($result =~ m/<item(.*?)<\/item>/sgi) {
		...
		if ($content =~ m/<upnp:rating>(.*)<\/upnp:rating>/si) {
			$item->setRating($1);
		}
		if ($content =~ m/<upnp:foreignMetaData>Star Rating: (.*)<\/upnp:foreignMetaData>/si) {
			$item->setUserRating($1);
		}
		if ($content =~ m/<dc:date>(.*)<\/dc:date>/si) {
			$item->setdate($1);
		}
		if ($content =~ m/<upnp:albumArtURI[^>]*>(.*?)<\/upnp:albumArtURI>/si) {
			$item->setPicture(Net::UPnP::HTTP::xmldecode($1));
		}
		if ($content =~ m/<res[^>]*>(.*?)<\/res>/si) {
			$item->seturl(Net::UPnP::HTTP::xmldecode($1));
		}
		if ($content =~ m/<res duration=\"([^"]+)\" bitrate=\"[^"]+\" size=\"([^"]+)\"/si) { #"
		  #print "\n\nSIZE: $2, DUR: $1\n\n";
			$item->setSize($2);
			$item->setDur($1);
		}
