...
##### Import libraries
  ...
  my $sageRecordings = '\\SIDECAR\Videos\SageRecordings';
  my $organizeFolder = '\\SIDECAR\Videos\Videos\TV';
  
  my @recordingsToOrganize = scanDir($sageRecordings,"properties");
  
  foreach $recording (@recordingsToOrganize)
  {
      $thumbnail = getFullFile($recording).".jpg";
      $propertiesFile = getFullFile($recording).".properties";
      $propertiesFile = getFullFile($recording).".edl";
      if (-e getFullFile($recording).".mkv")
      {
          $video = getFullFile($recording).".mkv";
      }
      elsif (-e getFullFile($recording).".avi")
      {
          $video = getFullFile($recording).".avi";    
      }
      $title       = getFile($exe);
      $thumbnail   = "";
      $parameters  = "";
